    ...
    <div class="nine wide column">
      ...
      <div class="ui segment px-4 py-4">
        ...
        <div class="ui feed blog-article-list rm-link-color">
          ...
          <div v-for="(item,index) in collect" :key="index" class="event">
            <div class="content">
              <div class="summary">
                <router-link :to="'/topics/'+item.topic.id"><a class="title"> {{ item.topic.title }} </a></router-link>
              </div>
              <div class="meta" style="line-height: 34px;">
                <div class="tags" style="margin: -2px 0px 0px;margin-bottom: -8px;">
                  <a v-for="(tag,i) in item.topic.tag" :key="i"  class="ui label small"
                ...
                <div class="date" style="font-size: 13px;margin: 7px 0em 0em;margin-left:0px">
                  ...
                  <span class="divider">|</span>
                  <a>
                    阅读数 {{item.topic.view_count}}
                  </a>
                  <span class="divider">|</span>
                  <a>
                    评论数 {{item.topic.reply_count}}
                  </a>
                  <span class="divider">|</span>
                  <a>
                    点赞数 {{item.topic.zan_count}}
                  </a>
                  <span class="divider">|</span>
                  <a>
                    收藏数 {{item.topic.collect_count}}
                  </a>
                </div>
              </div>
            </div>
  ...
  export default {
    ...
    methods:{
      changePage(page) {
        ...
        this.$router.push('/user-collect/'+id+'?page='+page);
      },
      init(){
        const id = this.$route.params.id
        ...
        http('user-collect/'+id,{page:this.currentPage},'GET',false).then(res =>{
